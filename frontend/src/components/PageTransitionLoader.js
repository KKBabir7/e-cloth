...
function PageTransitionLoaderContent() {
  ...
  useEffect(() => {
    ...
    // Smoothly finish progress bar to 100% and hide it quickly
    timerRef.current = setTimeout(() => {
      ...
      setTimeout(() => {
        setVisible(false);
        setProgress(0);
      }, 150); // fast transition fadeout
    }, 50); // complete instantly on load
  ...
  // Click handler to pre-start loading bar on internal link clicks
  useEffect(() => {
  ...
  return (
    <div
      style={{
        ...
        height: '3px',
        zIndex: 99999,
        backgroundColor: 'rgba(255, 133, 37, 0.15)', // Theme orange
        pointerEvents: 'none',
      }}
    >
      <div
        style={{
          height: '100%',
          width: `${progress}%`,
          background: 'linear-gradient(90deg, #ff8525, #e67316)',
          borderRadius: '0 3px 3px 0',
          transition: 'width 0.12s ease-out',
          boxShadow: '0 0 10px rgba(255, 133, 37, 0.6), 0 0 4px rgba(255, 133, 37, 0.4)',
        }}
      >
        <div
          style={{
