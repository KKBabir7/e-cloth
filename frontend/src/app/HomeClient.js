...
import { IoShirtOutline, IoCartOutline, IoHeartOutline, IoHeart, IoArrowForward, IoImageOutline, IoLayersOutline, IoFlashOutline, IoLockClosedOutline, IoRefreshOutline, IoStar, IoShieldCheckmarkOutline } from 'react-icons/io5';
...
export default function HomeClient({ initialProducts = [], initialNewArrivals = [], initialSlides = [], initialCategories = [] }) {
  ...
  const newArrivals = newArrivalsData || initialNewArrivals;
  ...
  const { data: slidesData } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        return res.data.success ? res.data.slides : [];
      } catch (err) {
        return [];
      }
    },
  ...
  const slides = slidesData || initialSlides;
