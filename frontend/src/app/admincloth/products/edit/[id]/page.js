...
import { 
  ...
  IoSearchOutline, IoCloudUploadOutline, IoLibraryOutline, IoCreateOutline
...
export default function EditProductPage() {
  ...
  const [colorImageTarget, setColorImageTarget] = useState(null);
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      setLoading(true);
      try {
        const res = await axios.get(`${getBackendUrl()}/api/products/${id}`);
        if (res.data.success && res.data.product) {
          ...
          setSizes(p.variants?.sizes || []);
          setColors(p.variants?.colors || []);
          // Load colorImages — the backend returns a Map which gets serialized to a plain object
          const rawColorImages = p.colorImages || {};
          const ciObj = rawColorImages instanceof Map ? Object.fromEntries(rawColorImages) : rawColorImages;
          setColorImages(ciObj);
          
          // Migrate old array of key-value specs to structured HTML string
          let specsHtml = '';
          if (Array.isArray(p.specifications)) {
            specsHtml = '<ul>' + p.specifications.map(s => `<li><strong>${s.key}:</strong> ${s.value}</li>`).join('') + '</ul>';
          } else {
            specsHtml = p.specifications || '';
          }
          setSpecifications(specsHtml);
        ...
        console.warn('Backend offline, loading mock product details for edit page');
        setName('Classic Crimson Polo Shirt');
        setSlug('classic-crimson-polo-shirt');
        setSku('CW-POLO-CRM-M');
        setCategory('Polo');
        setPrice(1250);
        setDiscountPrice(999);
        setStock(12);
        setDescription('Premium pique knit polo shirt featuring custom embroidery and a comfortable slim fit.');
        setImages(['https://images.unsplash.com/photo-1581655353564-df123a1eb820?w=500&auto=format&fit=crop']);
        setSizes(['M', 'L', 'XL']);
        setColors(['#dc2626', '#ffffff']);
        setSpecifications('<ul><li><strong>Fabric:</strong> Pique Cotton Blend</li><li><strong>Fit:</strong> Slim Fit</li></ul>');
        setStatus('Active');
        setFeatured(true);
        setTrending(false);
      } finally {
  ...
  const handleUpdateReview = async (reviewId) => {
    if (!editingReviewName.trim() || !editingReviewComment.trim()) {
      showToast('Name and comment cannot be empty!', 'error');
      return;
    }
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`${getBackendUrl()}/api/products/${id}`, payload);
      if (res.data.success) {
        showToast('Apparel changes updated successfully!', 'success');
  ...
  if (loading) {
    return (
      <div className="d-flex flex-column align-items-center justify-content-center py-5">
        ...
        <p className="mt-3 text-muted">Retrieving product record from database...</p>
      </div>
    );
  }
  ...
  return (
    <div className="pb-5">
      <div className="d-flex align-items-center justify-content-between mb-4">
        ...
        <h4 className="fw-bold mb-0 text-navy-gradient text-dark">Modify Premium Apparel Details</h4>
      </div>
      ...
      <Form onSubmit={handleSubmit}>
        <Row className="gy-4">
          <Col lg={8} className="d-flex flex-column gap-4">
                  ...
                  <div key={index} className="d-flex gap-3 align-items-center">
                    ...
                    <InputGroup className="flex-grow-1">
                      ...
                      <Button 
                        ...
                        className="d-flex align-items-center gap-1 px-3 text-muted"
                      ...
                      <Button 
                        ...
                        className="d-flex align-items-center gap-1 px-3 border-start-0 text-muted"
                        style={{ borderColor: '#e2e8f0', background: '#f8fafc' }}
                      >
            ...
            {/* Dynamic Product Reviews Management Card */}
            <Card className="border-0 shadow-sm p-4 bg-white rounded-3 mt-4">
              <h5 className="fw-bold mb-3 border-bottom pb-2" style={{ color: 'var(--primary-navy)' }}>Customer Reviews Management</h5>
              <p className="text-muted small mb-3">View and moderate customer reviews submitted for this product. Inappropriate or spam reviews can be edited or deleted permanently.</p>
              
              {reviews.length === 0 ? (
                <div className="text-muted small py-3 text-center border rounded-3 bg-light">
                ...
                <div className="d-flex flex-column gap-3">
                  {reviews.map((rev) => {
                    ...
                    return (
                      <div key={rev._id} className="p-3 border rounded-3 bg-light shadow-sm">
                        {isEditing ? (
                          // Edit Form Inline
                          <div className="d-flex flex-column gap-2">
                            <Form.Group>
                              <Form.Label className="small fw-bold">Reviewer Name</Form.Label>
                              <Form.Control
                                type="text"
                                value={editingReviewName}
                                onChange={(e) => setEditingReviewName(e.target.value)}
                                className="form-control-premium py-1 px-2"
                                style={{ fontSize: '13px' }}
                              />
                            </Form.Group>
                            <Form.Group>
                              ...
                              <Form.Select
                                ...
                                onChange={(e) => setEditingReviewRating(e.target.value)}
                                className="form-select-premium py-1 px-2"
                                style={{ fontSize: '13px' }}
                              >
                                {[1, 2, 3, 4, 5].map(n => (
                            ...
                            <div className="d-flex gap-2 justify-content-end mt-2">
                              ...
                              <Button 
                                ...
                                size="sm" 
                                variant="danger" 
                                className="bg-red-gradient border-0"
                          ...
                          <div className="d-flex justify-content-between align-items-start">
                            <div className="flex-grow-1">
                              <div className="d-flex align-items-center gap-2 mb-1">
                                <span className="fw-bold text-dark" style={{ fontSize: '13.5px' }}>{rev.name}</span>
                                <span className="text-warning small">{'★'.repeat(rev.rating)}{'☆'.repeat(5 - rev.rating)}</span>
                              ...
                              <p className="mb-0 text-muted small" style={{ fontStyle: 'italic' }}>"{rev.comment}"</p>
                            </div>
